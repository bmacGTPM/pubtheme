...
g = ggplot(dg, 
  ...
  labs(title    = title,
       ...
       caption  = "Optional caption with more info, Twitter handle, or shameless promotion of pubtheme",
...
dg = economics_long %>%
  mutate(name = toupper(variable), 
         ...
         x = as.character(date)) %>%
  select(x, name, y) %>%
  filter(x %in% range(x)) %>% ## keep first and last date
...
library(tidyverse)
library(ggtext)
library(pubtheme) ## https://github.com/bmacGTPM/pubtheme
...
df = df %>% 
  mutate(label = paste0('<img src="', 
                          image, 
                          '" style = "display:inline-block; vertical-align:middle" width="92" height="48">'))
df
```
