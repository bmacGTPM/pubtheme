...
```{r scatter.old, fig.height = 7, fig.width=7}
dg = mtcars %>% 
  select(wt, mpg, cyl) %>%
...
Note that the default is `base_size = 12`, which works well when viewing the image in Positron and when saving with `ggsave`. 

Do not change `width = 6`, `units  = 'in'`, or `dpi    = 300`. Height can be adjusted if desired. A square image is often preferred, so when in doubt, keep height at 6.
...
If you are using `size` inside `aes()`, you may need to adjust `scale_size()` so the points are a reasonable size. 
...
```{r scatter, fig.height = 7, fig.width = 7}
title = "Title in Upper Lower" 
g = ggplot(dg, 
           aes(x     = wt, 
               y     = MPG, 
...
```{r corrplot, fig.height = 7, fig.width = 6, }
...
```{r corrplot2, fig.height = 7, fig.width = 6}
...
```{r corrplot3, fig.height = 7, fig.width = 6}
library(ggcorrplot)
corr = cor(mtcars)
ds = as.dist(1-corr)
dh = hclust(ds)
...
```{r pairs, fig.height = 7, fig.width = 7}
...
Also note that `title`, `xlab`, and `ylab` are set via `ggpairs` arguments rather than `labs()`. As of ggplot2 4.0, `labs()` returns an object with class `ggplot2::labels` rather than the bare `labels` class that GGally 2.2.1 checks for, so `+ labs(...)` on a `ggmatrix` object throws an error. This also means subtitle and caption aren't currently supported for pairs plots, since `ggpairs` has no arguments for them and `labs()` can't be added directly. .
...
```{r line, fig.height = 7, fig.width = 7}
dg = economics_long %>%
...
```{r grid, fig.height = 4, fig.width = 7}
dg = airquality %>%
...
Note that we make the day numbers smaller by putting `size` *inside* `aes`. This is so that the text will auto resize when different `base_size` is used.  If `size` is outside of `aes`, it will remain that fixed number and won't auto resize with different `base_size`.

First, we start with a `data.frame` that has `date` and `value` (the var by which we'll color the heatmap) columns.
...
```{r cal, fig.height = 7, fig.width = 7}
...
```{r slope, fig.height = 6, fig.width = 6}
...
```{r dot.whiskers, fig.height = 7*10/20}
...
```{r timeline, fig.height = 7, fig.width = 5}
...
```{r colors, fig.height = 8, fig.width = 7}
dg = mtcars %>%
...
```{r maps, fig.width = 7, fig.height = 5}
...
```{r hexbins, fig.width = 7, fig.height = 8}
dg = economics
...
```{r hexbins2, fig.width = 7, fig.height = 8}
library(hexbin)
library(ggstar)
...
First let's choose a couple of images, and create a data frame with the image URLs, the country name and GDP per capita (taken from  https://en.wikipedia.org/wiki/List_of_countries_by_GDP_(nominal)_per_capita
on 11/21/2023).
...
You will likely need to change the width and height, but note that after saving the visualization to a file, the image will appear smaller than it appears on screen. 
...
`e_theme_pub()` applies the same colors, fonts, and type-specific axis rules as `theme_pub` to an `echarts4r` chart. Install `echarts4r` first (`install.packages("echarts4r")`), then call `e_theme_pub()` last in the pipeline.
...
GitHub README files cannot run JavaScript, so the figure below is a still. [Open the interactive echarts version](https://bmacgtpm.github.io/pubtheme/img/e-theme-pub-scatter.html).

[![echarts scatter](man/figures/README-echarts-scatter.png)](https://bmacgtpm.github.io/pubtheme/img/e-theme-pub-scatter.html)
...
A line chart. `type = "line"` keeps vertical grid lines off, matching `theme_pub`.
...
Use `colors = "cb14"` for the colorblind-friendly palette, or pass a vector of colors. For Shiny or R Markdown with several charts, register the theme once so it is not embedded in every widget:
...
`e_theme()` alone applies colors and fonts. `e_theme_pub()` also sets grid, axis, legend, and tooltip spacing for the plot `type`.
...
```{r plotly, fig.height = 20/3, fig.width = 20/3}
library(htmlwidgets)
dg = mtcars %>% 
  select(wt, 
         mpg, 
...
```{r ggplotly, fig.height = 20/3, fig.width = 20/3}
dg = mtcars %>% 
  select(wt, 
         mpg, 
         cyl) %>%
...
#   src = "img/Title in Upper Lower.html", 
#   scrolling = "no", 
#   seamless = "seamless",
#   frameBorder = "0"
# )
# htmltools::tags$iframe(
#   src = "https://bmacgtpm.github.io/pubtheme/img/Title%20in%20Upper%20Lower.html", 
#   scrolling = "no", 
#   seamless = "seamless",
#   frameBorder = "0"
# )
